...
// from kernel.org 6.15 by Raymond
...
#include <linux/gpio/consumer.h> // add by Raymond
...
struct es8326_priv {
    ...
    struct gpio_desc *spk_ctl_gpio; // add by Raymond
...
static int es8326_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    if (IS_ERR(es8326->spk_ctl_gpio)) {
        dev_err(&i2c->dev, "get spk_ctl_gpio failed");
        return PTR_ERR(es8326->spk_ctl_gpio);
    }
    gpiod_set_value(es8326->spk_ctl_gpio, 1);
    dev_info(&i2c->dev, "spk_ctl_gpio forced to 1\n"); // GDDBG new code doesn't require gpio any longger
